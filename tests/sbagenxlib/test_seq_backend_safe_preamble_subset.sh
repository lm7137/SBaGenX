...
TMP_REAL3_SBG="/tmp/sbx_safe_preamble_real3_$$.sbg"
TMP_REAL4="/tmp/sbx_safe_preamble_real4_$$.out"
ASSET_MIX="$ROOT_DIR/assets/river1.ogg"
trap 'rm -f "$TMP_SBG" "$TMP_SBG_INLINE" "$TMP_SBG_RUNTIME" "$TMP_OUT" "$TMP_OUT_INLINE" "$TMP_OUT_RUNTIME" "$TMP_REAL" "$TMP_REAL2" "$TMP_REAL3" "$TMP_REAL3_SBG" "$TMP_REAL4"' EXIT

if [[ ! -f "$ASSET_MIX" ]]; then
  echo "FAIL: missing asset mix file: $ASSET_MIX" >&2
  exit 1
fi
...
sed "s|-SE -m river1.ogg|-SE -m $ASSET_MIX|" "$ROOT_DIR/examples/basics/prog-drop-old-demo.sbg" >"$TMP_REAL3_SBG"
SBAGENX_SEQ_BACKEND=sbagenxlib "$BIN" -D "$TMP_REAL3_SBG" >"$TMP_REAL3"

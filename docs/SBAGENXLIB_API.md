...
instead of quantizing them to 16-bit first. When the available LAME runtime
exports its float-input entry point, the MP3 path now uses that same float
handoff for consistency; otherwise it falls back to the existing short-based
path. The same runtime path now also supports 24-bit uncompressed raw/WAV
export when `-b 24` is selected.
